...
static void write_elf_header(FILE *f, uint64_t base_vaddr, uint64_t code_off) {
...
static void write_program_header(FILE *f, uint64_t base_vaddr, uint64_t file_size) {
...
static size_t generate_code(uint8_t *code_buf, char * str) {
    ...
    memcpy(code_buf + len, str, str_len);
...
int compile(char *str) {
    ...
    const char *out = "out.elf";
    int fd = open(out, O_RDWR | O_CREAT, 0777);
    FILE *f = fdopen(fd, "wb");
    ...
    //
    // Load the whole file as one RX segment at vaddr 0x400000.
    const uint64_t base_vaddr = 0x400000;
    const uint64_t code_off   = 0x78;
    
    write_elf_header(f, base_vaddr, code_off);

    u8 code_buf[1024];
    size_t code_size = generate_code(code_buf, str);

    const uint64_t file_size = code_off + (uint64_t)code_size;

    write_program_header(f, base_vaddr, file_size);

    fprintf(stderr, "code size: %lu\n", code_size);


    // ---------------- Code/data ----------------
    // Ensure f is pointing at code_off (0x78)
    long pos = ftell(f);
    if (pos < 0) { 
        perror("ftell"); return 1; 
    }
    ...
    if (fwrite(code_buf, 1, code_size, f) != code_size) {
        perror("fwrite");
        return 1;
    }
